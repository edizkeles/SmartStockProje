...
import { savePurchaseLog } from '../store/slices/purchaseLogSlice';
...
export default function CartPanel() {
  ...
  const handleCheckout = async () => {
    ...
    try {
      // 1. Update stocks in API/database
      ...
      // 2. Add purchase log
      const itemsPurchased = cartItems.map((item) => ({
        productId: item.productId,
        ad: item.ad,
        quantity: item.quantity,
        pricePerUnit: item.pricePerUnit,
        totalItemPrice: item.quantity * item.pricePerUnit,
        isWeightBased: item.isWeightBased
      }));

      await dispatch(savePurchaseLog({
