...
import { saveStockUpdateLog } from '../store/slices/purchaseLogSlice';
...
export default function StockUpdateModal({ isOpen, onClose, product }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const parsedStockVal = parseFloat(stockVal);
      await dispatch(changeStock({ id: product.id, newStock: parsedStockVal })).unwrap();
      
      // Log the stock update event
      await dispatch(saveStockUpdateLog({
        username: currentUser?.username || 'admin',
        productName: product.ad,
        oldStock: product.stok_miktari,
        newStock: parsedStockVal,
        unit: product.urun_tipi === "Gida" ? "kg" : "Adet"
      })).unwrap();
